...
import { appendHistory, historyPath, learningHints, newHistoryId, newRunId, readHistory, setFeedback, similarity } from "../history.js";
...
test("handles disabled, missing, malformed, and individually rated history", () => {
  ...
  const disabled: HistoryConfig = { enabled: false, learningEnabled: true, similarityThreshold: 0.25, path: file };
  ...
  try {
    ...
    const record: HistoryRecord = {
      id: "one", timestamp: "2026-01-01T00:00:00.000Z", cwd: "/repo", task: "fix parser",
      agent: "claude", modelTier: "deep", model: "model", effort: "high", complexity: 2, exitCode: 0, durationMs: 1,
    };
...
test("derives positive and negative learning hints from similar feedback", () => {
  ...
  const config: HistoryConfig = { enabled: true, learningEnabled: true, similarityThreshold: 0.2, path: path.join(dir, "history.jsonl") };
  const base: HistoryRecord = {
    id: "good", timestamp: "2026-01-01T00:00:00.000Z", cwd: "/repo", task: "fix parser bug",
    agent: "claude", modelTier: "deep", model: "model", effort: "high", complexity: 2, exitCode: 0, durationMs: 1, feedback: "good",
  };
  try {
    appendHistory(config, base);
    appendHistory(config, { ...base, id: "bad", agent: "codex", modelTier: "fast", feedback: "bad" });
