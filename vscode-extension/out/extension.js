...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("airo.sidebar", provider, { webviewOptions: { retainContextWhenHidden: true } }), vscode.commands.registerCommand("airo.runTask", () => provider.focus()), vscode.commands.registerCommand("airo.openSettings", () => vscode.commands.executeCommand("workbench.action.openSettings", "@ext:pablospaniard.airo-vscode")), vscode.commands.registerCommand("airo.openTerminal", () => {
...
class SidebarProvider {
    ...
    focus() { this.view?.show?.(true); this.post({ type: "focus" }); }
    ...
    async action(action, text) {
        ...
        const commands = { models: ["models"], account: ["account"], usage: ["usage"], logs: ["logs"], doctor: ["doctor"] };
        if (commands[action])
    ...
    run(args, showOutput, label = args.join(" ")) {
        ...
        return new Promise(resolve => {
            ...
            const write = (data) => { const text = data.toString(); output += text; if (showOutput)
                this.post({ type: "output", text }); };
            child.stdout.on("data", write);
            child.stderr.on("data", write);
            child.on("error", error => this.notice("Could not start AIRO: " + error.message));
            child.on("close", code => { this.running = false; if (showOutput)
                this.post({ type: "end", code }); resolve({ code, output, started }); });
        });
    }
    settingSummary() {
        const config = vscode.workspace.getConfiguration("airo");
    ...
    notice(value) { this.post({ type: "notice", value }); }
